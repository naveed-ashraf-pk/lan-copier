...
Single `run(dest, src, src_path, dest_path, policy, method, on_ask, on_bytes, proc_sink, on_finish, on_merge)`.
...
- **Progress/stall/pause**: the existing tar pump & stall clock; `proc_sink` is the single pause/cancel channel (see pause rule §4.4). Progress accounting happens **as bytes leave the source reader**, before they reach whatever extracts them — so progress/ETA is identical regardless of the destination's OS; no per-OS progress code is needed. Follow-up (session 9): the UI caps the live bar below 100% until the transfer is marked done, and the *local*-destination dir-onto-dir merge reports per-entry progress via `on_merge(done, total)` (an additive, default-None callback threaded through `copy/_copy_legacy/_place/_merge_dir`); remote placement stays a single opaque command.
