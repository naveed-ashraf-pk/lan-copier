...
| `app/window.py` | **Current UI** (`AppWindow`): composes two side columns (each = one `EndpointBar` above a `DirPane`) in a horizontal Paned, a **vertical Paned** above a Notebook (Selected + Transfers + Log tabs), and a delete-progress stack page. Hosts the entire connection state-machine and the transfer/export/delete engine. The single `_on_connect_clicked(bar)` opens the `ConnectionDialog` popup for connect/change/disconnect of either side. Destination paths for a remote dest are built with `commands.paths.join(family, ...)`; selection changes cascade to the Selected tab + delete buttons; dest auto-reloads after transfers. Also drives the per-pane status-bar right labels: the **source label** (source disk space `X / Y free` when nothing is selected — no color; the **selection hint** `Selected: X (N files)`, green/red vs. the destination's free space, conflict-adjusted net, when items are checked) and the **destination disk space** display (`X / Y free` / `After copy: …`). Both are fed by lazy per-folder size calculations and per-side cached `disk_space` queries with drill-down reuse (`self._ds` — computed display state, cleared on swap/disconnect, never swapped). |
...
| `app/widgets/dirpane.py` | `DirPane`: symmetric browse **tree** shared by both sides (filter + quick-select `Missing`/`Changed`/`Invert`/`Folders`/`Files` **above the tree**, colourised TreeModelSort treeview with checkboxes, state column, and a `status_row`: left summary counts + spinner + right-aligned status label with optional color). Pure view — the window supplies items/states; the path bar lives on `EndpointBar`. Lazy folder sizes land via `set_folder_size` (updates the raw `COL_SIZE` + text + `folder_sizes` meta). |
| `commands/local.py` etc. | **`disk_space(path)`** on the endpoint contract (both transports): local via `shutil.disk_usage`, SSH POSIX via `df -B1 --output` (GNU) / `df -k` (macOS), SSH Windows via `[IO.DriveInfo]`; returns `{"total","free"}` or `None`. Builders in `commands/posix.py` (`df_gnu`/`df_darwin`) and `commands/powershell.py` (`disk_space`). |
...
- **Comparative State Engine (`classify_items`)**:
  ...
  - `differ` (Orange): Exists in both, but size differs — files compare their byte size; directories their recursive `(bytes, files)`, once both sides' lazy sizes have landed (pre-landing folders are provisionally `same`).
  - `conflict` (Magenta): Type mismatch (file on one side, directory on the other).
  - `same` (Green): Identical size / equal recursive `(bytes, files)` on both sides.
  - `extra` (Blue): Exists only in destination (informational / candidate for cleanup).
- While folder sizes are still calculating, an already-listed folder shows `same`; `AppWindow._reclassify_folder_state` promotes it to `differ` when both sides report equal/different recursive sizes. Folders with a `failed` or `partial` size result (undercount, e.g. permission denied mid-walk) keep the provisional `same` — the comparison never flips on an undercount. Local walks flag `partial` in `stat_remote`; SSH returns `None`→`failed` on a non-zero `find` rc.
