...
| `app/window.py` | **Current UI** (`AppWindow`): composes two side columns (each = one `EndpointBar` above a `DirPane`) in a horizontal Paned, a **vertical Paned** above a Notebook (Selected + Transfers + Log tabs), and a delete-progress stack page. Hosts the entire connection state-machine and the transfer/export/delete engine. The single `_on_connect_clicked(bar)` opens the `ConnectionDialog` popup for connect/change/disconnect of either side. Destination paths for a remote dest are built with `commands.paths.join(family, ...)`; selection changes cascade to the Selected tab + delete buttons; dest auto-reloads after transfers. Also drives the per-pane status-bar right labels: the **source selection hint** (`Selected: X (N files)`, green/red vs. the destination's free space, conflict-adjusted net) and the **destination disk space** display (`X / Y free` / `After copy: …`), both fed by lazy per-folder size calculations and a cached `disk_space` query with drill-down reuse. |
